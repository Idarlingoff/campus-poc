...
export async function createChallenge(userId: string, body: any) {
    ...
    const participationMode: ParticipationMode = isParticipationMode(body.participationMode ?? body.participation_mode)
        ? (body.participationMode ?? body.participation_mode)
        : "SOLO";

    const requiresProof = Boolean(body.requiresProof ?? body.requires_proof ?? false);
    ...
    if (!Number.isFinite(podiumSize) || podiumSize < 1 || podiumSize > 10) throw new Error("podiumSize invalid");

    if ((startAt && !endAt) || (!startAt && endAt)) throw new Error("startAt/endAt required together");
 ...
 */
export async function registerToChallenge(userId: string, challengeId: string, body: any) {
 ...
 */
export async function submitProof(userId: string, challengeId: string, body: any) {
 ...
 */
export async function setWinners(moderatorId: string, challengeId: string, body: any) {
    ...
    try {
        ...
        const chRes = await client.query(`select * from challenges where id=$1 and status='APPROVED'`, [challengeId]);
        const ch = chRes.rows[0];
        if (!ch) throw new Error("challenge not found");
        ...
        for (const r of ranks) {
            if (!Number.isFinite(r) || r < 1 || r > podiumSize) throw new Error("rank invalid");
        }
        ...
        await client.query(`delete from challenge_winners where challenge_id=$1`, [challengeId]);
...
export async function getChallengeForModerationById(_moderatorId: string, id: string) {
