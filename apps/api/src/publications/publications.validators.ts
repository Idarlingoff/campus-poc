export function validateCreatePublication(body: any) {
    ...
    if (!allowedVisibility.includes(visibility)) throw new Error('Invalid visibility');

    if (type === 'EVENT') {
        if (!eventStartAt || !eventEndAt) throw new Error('Event start/end required');
    }

    return { type, title, contentHtml, visibility, campusId, themeId, eventStartAt, eventEndAt };
}

export function validatePatchPublication(body: any) {
    const out: any = {};
    if (body.title !== undefined) out.title = String(body.title).trim();
    if (body.contentHtml !== undefined) out.contentHtml = String(body.contentHtml).trim();
    if (body.visibility !== undefined) out.visibility = String(body.visibility);
    if (body.campusId !== undefined) out.campusId = body.campusId ? String(body.campusId) : null;
    if (body.themeId !== undefined) out.themeId = body.themeId ? String(body.themeId) : null;
    if (body.eventStartAt !== undefined) out.eventStartAt = body.eventStartAt ? String(body.eventStartAt) : null;
    if (body.eventEndAt !== undefined) out.eventEndAt = body.eventEndAt ? String(body.eventEndAt) : null;
    return out;
}
